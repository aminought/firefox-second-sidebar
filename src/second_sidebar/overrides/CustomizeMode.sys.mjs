...
import { DragPositionManager } from "./DragPositionManager.sys.mjs";
...
window.XPCOMUtils.defineLazyServiceGetter(
  lazy,
  "gTouchBarUpdater",
...
export class CustomizeMode {
  ...
  enter() {
    ...
    (async () => {
      ...
      window.CustomizableUI.dispatchToolboxEvent(
        "beforecustomization",
        {},
        window,
      );
      window.CustomizableUI.notifyStartCustomizing(this.#window);
      ...
      window.CustomizableUI.addListener(this);
  ...
  exit() {
    ...
    (async () => {
      ...
      window.CustomizableUI.notifyEndCustomizing(window);
  ...
  #getCustomizableChildForNode(aNode) {
    ...
    let areas = window.CustomizableUI.areas;
    ...
    for (let i = 0; i < numberOfAreas; i++) {
      ...
      let customizationTarget =
        window.CustomizableUI.getCustomizationTarget(areaNode);
  ...
  async addToToolbar(aNode) {
    ...
    if (
      window.CustomizableUI.isSpecialWidget(widgetToAdd) &&
    ...
    window.CustomizableUI.addWidgetToArea(
      widgetToAdd,
      window.CustomizableUI.AREA_NAVBAR,
    );
    ...
    if (!this.#customizing) {
      window.CustomizableUI.dispatchToolboxEvent("customizationchange");
    }
  ...
  async addToPanel(aNode, aReason) {
    ...
    let panel = window.CustomizableUI.AREA_FIXED_OVERFLOW_PANEL;
    window.CustomizableUI.addWidgetToArea(aNode.id, panel);
  ...
  async removeFromArea(aNode, aReason) {
    ...
    window.CustomizableUI.removeWidgetFromArea(aNode.id);
  ...
  #populatePalette() {
    ...
    try {
      let unusedWidgets =
        window.CustomizableUI.getUnusedWidgets(toolboxPalette);
      ...
      let flexSpace = window.CustomizableUI.createSpecialWidget(
        "spring",
        this.#document,
      );
  ...
  createOrUpdateWrapper(aNode, aPlace, aIsUpdate) {
    ...
    let removable =
      aPlace == "palette" || window.CustomizableUI.isWidgetRemovable(aNode);
  ...
  async #wrapAreaItems(aArea) {
    let target = window.CustomizableUI.getCustomizeTargetForArea(
      aArea,
      this.#window,
    );
  ...
  async #wrapAllAreaItems() {
    for (let area of window.CustomizableUI.areas) {
  ...
  #addCustomizeTargetDragAndDropHandlers(aTarget) {
    ...
    if (aTarget.id == window.CustomizableUI.AREA_FIXED_OVERFLOW_PANEL) {
  ...
  undoReset() {
    ...
    return (async () => {
      ...
      window.CustomizableUI.undoReset();
  ...
  onWidgetAfterDOMChange(aNodeToChange, aSecondaryNode, aContainer) {
    ...
    if (aNodeToChange.parentNode) {
      ...
      let widget = window.CustomizableUI.getWidget(widgetId);
      if (widget.provider == window.CustomizableUI.PROVIDER_API) {
        let paletteItem = this.#makePaletteItem(widget);
  ...
  onWidgetAfterCreation(aWidgetId, aArea) {
    ...
    if (!aArea) {
      ...
      if (widgetNode) {
        ...
        let widget = window.CustomizableUI.getWidget(aWidgetId);
   ...
   *   The DOM node for the customizable area.
   * @param {string} aReason
   *   One of the window.CustomizableUI.REASON_* constants to describe the reason
   *   that the area was unregistered for.
   */
  onAreaNodeUnregistered(aArea, aContainer, aReason) {
    if (
      aContainer.ownerDocument == this.#document &&
      aReason == window.CustomizableUI.REASON_AREA_UNREGISTERED
    ) {
  ...
  #updateResetButton() {
    let btn = this.$("customization-reset-button");
    btn.disabled = window.CustomizableUI.inDefaultState;
  }
  ...
  #attachEventListeners() {
    ...
    this.$(kPaletteItemContextMenu).addEventListener(
      "popupshowing",
      (event) => {
  ...
  #onDragOver(aEvent, aOverrideTarget) {
    ...
    if (
      targetNode == window.CustomizableUI.getCustomizationTarget(targetArea)
    ) {
    ...
    if (
      ...
      if (
        dragOverItem != window.CustomizableUI.getCustomizationTarget(targetArea)
      ) {
        this.#setDragActive(
          dragOverItem,
          dragValue,
  ...
  #applyDrop(aEvent, aTargetArea, aOriginArea, aDroppedItemId, aTargetNode) {
    ...
    if (aTargetArea.id == kPaletteId) {
      ...
      if (aOriginArea.id !== kPaletteId) {
        ...
        window.CustomizableUI.removeWidgetFromArea(aDroppedItemId, "drag");
        lazy.BrowserUsageTelemetry.recordWidgetChange(
          aDroppedItemId,
          null,
          "drag",
    ...
    let areaCustomizationTarget =
      window.CustomizableUI.getCustomizationTarget(aTargetArea);
    ...
    if (
      window.CustomizableUI.isSpecialWidget(aDroppedItemId) &&
    ...
    if (aTargetNode == areaCustomizationTarget) {
      window.CustomizableUI.addWidgetToArea(aDroppedItemId, aTargetArea.id);
      lazy.BrowserUsageTelemetry.recordWidgetChange(
        aDroppedItemId,
        aTargetArea.id,
        "drag",
    ...
    if (itemForPlacement) {
      let targetNodeId =
        itemForPlacement.nodeName == "toolbarpaletteitem"
          ...
          : itemForPlacement.id;
      placement = window.CustomizableUI.getPlacementOfWidget(targetNodeId);
    }
    if (!placement) {
      lazy.log.debug(
    ...
    if (aTargetArea == aOriginArea) {
      window.CustomizableUI.moveWidgetWithinArea(aDroppedItemId, position);
      lazy.BrowserUsageTelemetry.recordWidgetChange(
        aDroppedItemId,
        aTargetArea.id,
        "drag",
  ...
  #cancelDragActive(aDraggedOverItem, aNextDraggedOverItem, aNoTransition) {
    ...
    let areaType = window.CustomizableUI.getAreaType(currentArea.id);
    if (areaType) {
      if (aNoTransition) {
        aDraggedOverItem.setAttribute("notransition", "true");
      }
  ...
  #getDragItemSize(aDragOverNode, aDraggedItem) {
    ...
    if (targetArea != currentArea) {
      ...
      areaType = window.CustomizableUI.getAreaType(targetArea.id);
      currentType =
      ...
      window.CustomizableUI.onWidgetDrag(aDraggedItem.id, targetArea.id);
    } else {
    ...
    if (targetArea != currentArea) {
      ...
      window.CustomizableUI.onWidgetDrag(aDraggedItem.id);
    } else {
  ...
  #getDragOverNode(aEvent, aAreaElement, aPlace) {
    let expectedParent =
      window.CustomizableUI.getCustomizationTarget(aAreaElement) ||
      aAreaElement;
  ...
  #maybeMoveDownloadsButtonToNavBar() {
    ...
    if (
      ...
      let navbarPlacements =
        window.CustomizableUI.getWidgetIdsInArea("nav-bar");
...
function getPlaceForItem(aElement) {
  ...
  while (node && !place) {
    if (node.id == "sb2-main") {
      ...
      place = "toolbar";
    } else if (node.id == window.CustomizableUI.AREA_FIXED_OVERFLOW_PANEL) {
      place = "panel";
